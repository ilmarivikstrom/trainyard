...
class IndicatorStyle:
    def __init__(self, fg_active_color: Tuple[int, int, int]=GRAY5, bg_active_color: Tuple[int, int, int]=TRAIN_YELLOW, fg_deactive_color: Tuple[int, int, int]=GRAY50, bg_deactive_color: Tuple[int, int, int]=GRAY5):
...
class IndicatorItem:
    def __init__(self, font: pg.Font, text: str, padding_spaces: int, style: IndicatorStyle, dest: Tuple[int, int]) -> None:
    ...
    def render(self):
        if self.activated:
            self.renderable = self.font.render(f"{self.text:^{len(self.text) + 2 * self.padding_spaces}}", True, self.style.fg_active_color, self.style.bg_active_color)
        else:
